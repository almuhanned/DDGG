...
class wizard_inventory_valuation(models.TransientModel):
    ...
    def print_xls_report(self):
        self.check_date_range()
        xls_filename = 'inventory_valuation_reports.xlsx'
            ...
            if not self.location_ids:
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product,ending_qty,warehouse,'end')
                        ...
                        worksheet.write(rows, 3, formatLang(self.env,beginning_qty), header_data_format)
                        worksheet.write(rows, 4, formatLang(self.env, beginning_qty_val), header_data_format)
                        worksheet.write(rows, 5, formatLang(self.env,product_qty_in), header_data_format)
                        worksheet.write(rows, 6, formatLang(self.env, product_qty_in_val), header_data_format)
                        worksheet.write(rows, 7, formatLang(self.env,product_qty_out), header_data_format)
                        worksheet.write(rows, 8, formatLang(self.env, product_qty_out_val), header_data_format)
                        worksheet.write(rows, 9, formatLang(self.env,product_qty_internal), header_data_format)
                        worksheet.write(rows, 10, formatLang(self.env, product_qty_internal_val), header_data_format)
                        worksheet.write(rows, 11, formatLang(self.env,product_qty_adjustment), header_data_format)
                        worksheet.write(rows, 12, formatLang(self.env, product_qty_adjustment_val), header_data_format)
                        worksheet.write(rows, 13, formatLang(self.env,ending_qty), header_data_format)
                    ...
                    worksheet.write(rows + 1, 3, formatLang(self.env,prod_beginning_qty), header_merge_format)
                    worksheet.write(rows + 1, 4, formatLang(self.env, prod_beginning_qty_val), header_merge_format)
                    worksheet.write(rows + 1, 5, formatLang(self.env,prod_qty_in), header_merge_format)
                    worksheet.write(rows + 1, 6, formatLang(self.env, prod_qty_in_val), header_merge_format)
                    worksheet.write(rows + 1, 7, formatLang(self.env,prod_qty_out), header_merge_format)
                    worksheet.write(rows + 1, 8, formatLang(self.env, prod_qty_out_val), header_merge_format)
                    worksheet.write(rows + 1, 9, formatLang(self.env,prod_qty_int), header_merge_format)
                    worksheet.write(rows + 1, 10, formatLang(self.env, prod_qty_int_val), header_merge_format)
                    worksheet.write(rows + 1, 11, formatLang(self.env,prod_qty_adjust), header_merge_format)
                    worksheet.write(rows + 1, 12, formatLang(self.env, prod_qty_adjust_val), header_merge_format)
                    worksheet.write(rows + 1, 13, formatLang(self.env,prod_ending_qty), header_merge_format)
                    ...
                    for categ, product_value in product_val.items():
                        ...
                        for product in product_value:
                            ...
                            ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product_id,ending_qty,warehouse,'end')
                        ...
                        worksheet.write(rows, 3, formatLang(self.env,categ_prod_beginning_qty), header_merge_format)
                        ...
                        worksheet.write(rows, 7, formatLang(self.env,categ_prod_qty_out), header_merge_format)
                        worksheet.write(rows, 8, formatLang(self.env, categ_prod_qty_out_val), header_merge_format)
                        worksheet.write(rows, 9, formatLang(self.env,categ_prod_qty_int), header_merge_format)
                        worksheet.write(rows, 10, formatLang(self.env, categ_prod_qty_int_val), header_merge_format)
                        worksheet.write(rows, 11, formatLang(self.env,categ_prod_qty_adjust), header_merge_format)
                        worksheet.write(rows, 12, formatLang(self.env, categ_prod_qty_adjust_val), header_merge_format)
                        worksheet.write(rows, 13, formatLang(self.env,categ_prod_ending_qty), header_merge_format)
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        beginning_qty_val = report_stock_inv_obj.get_product_valuation(self,product,beginning_qty,warehouse,'beg',location_ids)
                        product_qty_in = location_wise_data[1][1]
                        product_qty_in_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_in,warehouse,'in',location_ids)
                        product_qty_out = location_wise_data[1][2]
                        product_qty_out_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_out,warehouse,'out',location_ids)
                        product_qty_internal = location_wise_data[1][3]
                        product_qty_internal_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_internal,warehouse,'int',location_ids)
                        product_qty_adjustment = location_wise_data[1][4]
                        product_qty_adjustment_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_adjustment,warehouse,'adj',location_ids)
                        ending_qty = location_wise_data[1][5]
                        ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product,ending_qty,warehouse,'end',location_ids)

                        worksheet.merge_range(rows, 0, rows, 1, product.display_name, product_header_format)
                        cost_method =  dict(product.categ_id.fields_get()['property_cost_method']['selection'])[product.categ_id.property_cost_method]
                        worksheet.write(rows, 2, cost_method, header_data_format)
                        worksheet.write(rows, 3, ' ', header_data_format)
                        worksheet.write(rows, 4, formatLang(self.env,beginning_qty), header_merge_format)
                        worksheet.write(rows, 5, formatLang(self.env, beginning_qty_val), header_merge_format)
                        worksheet.write(rows, 6, formatLang(self.env,product_qty_in), header_merge_format)
                        worksheet.write(rows, 7, formatLang(self.env, product_qty_in_val), header_merge_format)
                        worksheet.write(rows, 8, formatLang(self.env,product_qty_out), header_merge_format)
                        worksheet.write(rows, 9, formatLang(self.env, product_qty_out_val), header_merge_format)
                        worksheet.write(rows, 10, formatLang(self.env,product_qty_internal), header_merge_format)
                        worksheet.write(rows, 11, formatLang(self.env, product_qty_internal_val), header_merge_format)
                        worksheet.write(rows, 12, formatLang(self.env,product_qty_adjustment), header_merge_format)
                        worksheet.write(rows, 13, formatLang(self.env, product_qty_adjustment_val), header_merge_format)
                        worksheet.write(rows, 14, formatLang(self.env,ending_qty), header_merge_format)
                        ...
                        for location, value in location_wise_data[0].items():
                            ...
                            worksheet.write(rows, 4, formatLang(self.env,value[0]), header_data_format)
                            worksheet.write(rows, 5, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[0],warehouse,'beg',location)), header_data_format)
                            worksheet.write(rows, 6, formatLang(self.env,value[1]), header_data_format)
                            worksheet.write(rows, 7, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[1],warehouse,'in',location)), header_data_format)
                            worksheet.write(rows, 8, formatLang(self.env,value[2]), header_data_format)
                            worksheet.write(rows, 9, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[2],warehouse,'out',location)), header_data_format)
                            worksheet.write(rows, 10, formatLang(self.env,value[3]), header_data_format)
                            worksheet.write(rows, 11, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[3],warehouse,'int',location)), header_data_format)
                            worksheet.write(rows, 12, formatLang(self.env,value[4]), header_data_format)
                            worksheet.write(rows, 13, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[4],warehouse,'adj',location)), header_data_format)
                            worksheet.write(rows, 14, formatLang(self.env,value[5]), header_data_format)
                            worksheet.write(rows, 15, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[5],warehouse,'end',location)), header_data_format)
                            rows += 1
                    ...
                    worksheet.write(rows, 4, formatLang(self.env,prod_beginning_qty), header_merge_format)
                    worksheet.write(rows, 5, formatLang(self.env, prod_beginning_qty_val), header_merge_format)
                    worksheet.write(rows, 6, formatLang(self.env,prod_qty_in), header_merge_format)
                    worksheet.write(rows, 7, formatLang(self.env, prod_qty_in_val), header_merge_format)
                    worksheet.write(rows, 8, formatLang(self.env,prod_qty_out), header_merge_format)
                    worksheet.write(rows, 9, formatLang(self.env, prod_qty_out_val), header_merge_format)
                    worksheet.write(rows, 10, formatLang(self.env,prod_qty_int), header_merge_format)
                    worksheet.write(rows, 11, formatLang(self.env, prod_qty_int_val), header_merge_format)
                    worksheet.write(rows, 12, formatLang(self.env,prod_qty_adjust), header_merge_format)
                    worksheet.write(rows, 13, formatLang(self.env, prod_qty_adjust_val), header_merge_format)
                    worksheet.write(rows, 14, formatLang(self.env,prod_ending_qty), header_merge_format)
                    ...
                    for categ, product_value in product_val.items():
                        ...
                        for product in product_value:
                            ...
                            beginning_qty = location_wise_data[1][0]
                            beginning_qty_val = report_stock_inv_obj.get_product_valuation(self,product_id,beginning_qty,warehouse,'beg',location_ids)
                            product_qty_in = location_wise_data[1][1]
                            product_qty_in_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_in,warehouse,'in',location_ids)
                            product_qty_out = location_wise_data[1][2]
                            product_qty_out_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_out,warehouse,'out',location_ids)
                            product_qty_internal = location_wise_data[1][3]
                            product_qty_internal_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_internal,warehouse,'int',location_ids)
                            product_qty_adjustment = location_wise_data[1][4]
                            product_qty_adjustment_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_adjustment,warehouse,'adj',location_ids)
                            ending_qty = location_wise_data[1][5]
                            ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product_id,ending_qty,warehouse,'end',location_ids)
                            ...
                            for location, value in location_wise_data[0].items():
                                ...
                                worksheet.write(rows, 3, location.display_name, header_data_format)
                                worksheet.write(rows, 4, formatLang(self.env,value[0]), header_data_format)
                                worksheet.write(rows, 5, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[0],warehouse,'beg',location)), header_data_format)
                                worksheet.write(rows, 6, formatLang(self.env,value[1]), header_data_format)
                                worksheet.write(rows, 7, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[1],warehouse,'in',location)), header_data_format)
                                worksheet.write(rows, 8, formatLang(self.env,value[2]), header_data_format)
                                worksheet.write(rows, 9, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[2],warehouse,'out',location)), header_data_format)
                                worksheet.write(rows, 10, formatLang(self.env,value[3]), header_data_format)
                                worksheet.write(rows, 11, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[3],warehouse,'int',location)), header_data_format)
                                worksheet.write(rows, 12, formatLang(self.env,value[4]), header_data_format)
                                worksheet.write(rows, 13, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[4],warehouse,'adj',location)), header_data_format)
                                worksheet.write(rows, 14, formatLang(self.env,value[5]), header_data_format)
                                worksheet.write(rows, 15, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[5],warehouse,'end',location)), header_data_format)
                                rows += 1
                        ...
                        worksheet.merge_range(rows, 0, rows, 3, 'Total', header_merge_format)
                        worksheet.write(rows, 4, formatLang(self.env,categ_prod_beginning_qty), header_merge_format)
                        worksheet.write(rows, 5, formatLang(self.env, categ_prod_beginning_qty_val), header_merge_format)
                        worksheet.write(rows, 6, formatLang(self.env,categ_prod_qty_in), header_merge_format)
                        worksheet.write(rows, 7, formatLang(self.env, categ_prod_qty_in_val), header_merge_format)
                        worksheet.write(rows, 8, formatLang(self.env,categ_prod_qty_out), header_merge_format)
                        worksheet.write(rows, 9, formatLang(self.env, categ_prod_qty_out_val), header_merge_format)
                        worksheet.write(rows, 10, formatLang(self.env,categ_prod_qty_int), header_merge_format)
                        worksheet.write(rows, 11, formatLang(self.env, categ_prod_qty_int_val), header_merge_format)
                        worksheet.write(rows, 12, formatLang(self.env,categ_prod_qty_adjust), header_merge_format)
                        worksheet.write(rows, 13, formatLang(self.env, categ_prod_qty_adjust_val), header_merge_format)
                        worksheet.write(rows, 14, formatLang(self.env,categ_prod_ending_qty), header_merge_format)
