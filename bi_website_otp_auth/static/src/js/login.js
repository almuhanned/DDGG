odoo.define('bi_website_otp_auth.login', function (require) {
"use strict";
    ...
    publicWidget.registry.LoginForm = publicWidget.Widget.extend({
        ...
        start: function(){
            ...
            if($otp_option.length > 0){
                $(this.$el).find('.form-group.field-password').css('display','none');
        ...
        _OnNextButtonClick: function(ev){
            ...
            if(!$email){
                ...
                if($radio.val() === 'pwd'){
                    ...
                    $('.form-group.field-password').css('display','block');
                    $('button[type="submit"]').css('display','block');
                }
                if($radio.val() === 'otp'){
                    ajax.jsonRpc('/check/login', 'call', 
                        {'login':$email,
                    }).then(function (login) {
                        if(login === true){
        ...
        VerifyOtp: function($otp_type, $limit, $email){
            if($otp_type === 'text'){
                $('.form-group.field-password input').attr('type','text');;
            }
            $('.form-group.field-otp').css('display','none');
            $('.otp_next_btn').css('display','none');
            $('.form-group.field-password label').text('Enter Otp');
            $('.form-group.field-password input').attr('placeholder','Enter Otp');;
            $('.form-group.field-password').css('display','block');
            $('button[type="submit"]').css('display','block');
            $('.form-group.field-password').after('<p id="otp_msg" class="alert alert-success">OTP has been sent to given Email Address : '+ $email +'.</p>');
                ...
                if (result){
                    var Timer = setInterval(function(){
                        if($limit < 0){                 
                            ...
                            $('#otp_msg').remove();
                            $('.form-group.field-password').after('<a class="btn btn-link float-right otp_resend" href="#">Resend OTP</a>')
