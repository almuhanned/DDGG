...
class ir_rule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        read_value = True
        self._cr.execute("SELECT state FROM ir_module_module WHERE name='access_management'")
        ...
        value = self._cr.execute("""SELECT value from ir_config_parameter where key='uninstall_access_management' """)
        value = self._cr.fetchone()
        if not value:
            value = self._cr.execute("""select state from ir_module_module where name = 'access_management'""")
            value = self._cr.fetchone()
                ...
                if model_numeric_id and isinstance(model_numeric_id,int) and self.env.user:
                    ...
                    if access_domain_ah_ids:
                        ...
                        if model_name == 'res.partner':
                            ...
                            # domain_list = [[('id','in',self.env.user.partner_id.ids)]]
