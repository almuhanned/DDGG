    ...
    def _compute_input_line_ids(self):
        ...
        for rec in self:
            ...
            overtime_id = self.env['hr.overtime'].search(
                [('date_from', '>=', payslip_from_datetime_min), ('date_to', '<=', payslip_to_datetime_max),
                 ('employee_id', '=', rec.employee_id.id), ('contract_id', '=', rec.contract_id.id),
            ...
            if old_input_rec:
                print(old_input_rec)
                for rec_line in old_input_rec:
                    rec.input_line_ids = [(2, rec_line.id, 0)]
            if overtime_id and rec.struct_id and overtime_input_type in rec.struct_id.input_line_type_ids:
                rec.overtime_ids = overtime_id
