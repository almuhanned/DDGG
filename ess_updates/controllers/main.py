...
class ESSPortal(Controller):
    ...
    def check_modules(self):
        values = {}
        if request.env['ir.module.module'].sudo().search([('name', '=', 'dev_hr_loan')]).state == 'installed':
            values.update({
                'loan': True,
            })
        else:
    ...
    def loan(self, redirect=None, **post):
        ...
        if post and request.httprequest.method == 'POST':
            ...
            loan = request.env['employee.loan'].sudo().create(post)
            loan.compute_installment()

        loan_obj = request.env['employee.loan'].sudo().search([('employee_id','=',emb_obj.id)])
    ...
    def loan_report(self, redirect=None, **post):
        ...
        loan_obj = request.env['employee.loan'].sudo().search([('employee_id','=',emb_obj.id), ('state', '=', 'done')])
