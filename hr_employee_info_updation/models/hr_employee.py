...
class HrEmployee(models.Model):
    ...
    certificate_id = fields.Many2one('certificate.level', 'Certificate Level')
    job_history_ids = fields.One2many('hr.history.line', 'hr_employee_id')
    employee_child_ids = fields.One2many('hr.employee.child', 'hr_employee_id')
    employee_eos_amount_ids = fields.One2many('hr.employee.eos.line', 'hr_employee_id')
    contract_attachment_ids = fields.One2many('hr.employee.contract.attachment', 'hr_employee_id')
    ...
    children = fields.Integer(string='Number of Children', compute="get_number_of_children")

    @api.depends("employee_child_ids")
    def get_number_of_children(self):
        for record in self:
            record.children = len(record.employee_child_ids)
...
class HrEmployeeChild(models.Model):
    ...
    hr_employee_id = fields.Many2one('hr.employee')
    name = fields.Char()
    child_id = fields.Char(string='id')
    date_of_birth = fields.Date()
    gender = fields.Selection([
        ('male', 'male'),
        ('female', 'female')
    ], 'gender', default='male')
    relation_ship = fields.Char(string="relation")
    license_number = fields.Char()
    insurance_start_date = fields.Date()
    insurance_category = fields.Char()
    insurance_end_date = fields.Date()


class HrEmployeeEos(models.Model):
    _name = 'hr.employee.eos.line'
...
class HrEmployeeContractAttachment(models.Model):
    _name = 'hr.employee.contract.attachment'
