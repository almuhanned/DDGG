...
class PurchaseOrdergLine(models.Model):
    _inherit = "purchase.order.line"

    def name_get(self):
        result = []
        for rec in self:
            name = ''.join([rec.order_id.name, ' - ', str(rec.name)])
...
class StockMoveLine(models.Model):
    ...
    purchase_line_id = fields.Many2one('purchase.order.line', 'Purchase Order Line', related="move_id.purchase_line_id" ,store=True)
    sale_line_id = fields.Many2one('sale.order.line', 'Sale Order Line', related="move_id.sale_line_id",store=True )

    contact = fields.Many2one('res.partner', related="picking_id.partner_id", string="Contact")
    sale_price_unit = fields.Float('Sale Unit Price', required=True, related="sale_line_id.price_unit", store=True,digits='Product Price', default=0)
    purchase_price_unit = fields.Float('Purchase Unit Price', required=True, related="purchase_line_id.price_unit", store=True,digits='Product Price',default=0)


    # @api.depends('purchase_line_id','sale_line_id')
    # def _sale_price(self):
    #     self.sale_price_unit= 0
    #     self.purchase_price_unit= 0
    #     for rec in self:
    #         if rec.purchase_line_id:
    #             rec.purchase_price_unit = rec.purchase_line_id.price_unit
    #             return rec.purchase_price_unit
    #         if rec.sale_line_id:
    #             rec.sale_price_unit = rec.purchase_line_id.price_unit
    #             return rec.sale_price_unit
...
class Product(models.Model):
    ...
    #     self.ensure_one()
    #     action = self.env["ir.actions.actions"]._for_xml_id("product_info.stock_move_line_action_inherit")
...
class KartuStokReport(models.AbstractModel):
    _name = 'report.product_info.print_kartu_stok_proweb'
...
class KartuStokWizard(models.TransientModel):
    ...
    def action_kartu_stok_pdf_wizard(self):
        ...
        return self.env.ref('product_info.action_print_kartu_stok_proweb').report_action(active_ids, data=data)
